                        ...
                        <form method="POST" action="{{ route('candidate.profile.delete.confirm') }}" id="deleteProfileForm">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="reason" id="duplication" value="Duplication">
                                    <label class="form-check-label" for="duplication">Duplication</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="reason" id="retired" value="Retired / Quit sailing">
                                    <label class="form-check-label" for="retired">Retired / Quit sailing</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="reason" id="unsatisfactory" value="Service Unsatisfactory">
                                    <label class="form-check-label" for="unsatisfactory">Service Unsatisfactory</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="reason" id="not-required" value="Service no longer required">
                                ...
                                <div id="otherReasonBox" style="display:none;">
                                    <textarea name="other_reason" class="form-control bg-white bg-opacity-25 border-1 border-warning mt-2" rows="3" placeholder="Other (comment box for reason)"></textarea>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!selectedReason) {
            e.preventDefault();
            showToast('Please select a reason for deleting your profile.', 'error');
            return;
        }
        ...
        if (selectedReason.value === 'Other') {
            ...
            if (!otherReason) {
                e.preventDefault();
                showToast('Please provide your specific reason.', 'error');
                return;
            }
        }
    ...
    // Auto-hide success/error alerts after 8 seconds
...
// Toast notification function (alternative to alerts)
function showToast(message, type = 'info') {
    const toastContainer = document.createElement('div');
    toastContainer.className = `toast-notification toast-${type}`;
    toastContainer.innerHTML = `
        <div class="toast-content">
            <i class="bx bx-${type === 'error' ? 'error-circle' : 'info-circle'} me-2"></i>
            ${message}
        </div>
    `;
