{{-- resources/views/admin/hotjobs/edit.blade.php --}}
...
<main class="page-content professional-bg">
    <div class="container-fluid px-4">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="modern-breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">
                            ...
                            <span class="breadcrumb-text">Edit Hot Job</span>
                        </li>
                    </ol>
                </nav>
            </div>
        ...
        <!-- Modern Form Card -->
        <form action="{{ route('admin.hotjobs.update', $hotjob->id) }}" method="POST" class="modern-professional-card mb-4" autocomplete="off">
            @csrf
            @method('PUT')
            <div class="card-header modern-header">
                <div class="header-content">
                    ...
                    <div class="header-text">
                        <h5 class="header-title">Edit Maritime Hot Job</h5>
                        <p class="header-subtitle">Update job posting details for {{ $hotjob->company->company_name ?? 'the selected company' }}</p>
                    </div>
                </div>
            </div>
                                    ...
                                    <div class="input-container">
                                        <select name="company_id" class="modern-form-control modern-form-select @error('company_id') is-invalid @enderror" required>
                                            ...
                                            @foreach($companies as $company)
                                                <option value="{{ $company->id }}" {{ old('company_id', $hotjob->company_id) == $company->id ? 'selected' : '' }}>{{ $company->company_name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="input-container">
                                        <select name="rank_id" class="modern-form-control modern-form-select @error('rank_id') is-invalid @enderror" required>
                                            ...
                                            @foreach($ranks as $rank)
                                                <option value="{{ $rank->id }}" {{ old('rank_id', $hotjob->rank_id) == $rank->id ? 'selected' : '' }}>{{ $rank->rank }}</option>
                                            @endforeach
                                        </select>
                                        <div class="validation-icon">
                                            <i class="bx bx-check-circle valid-icon"></i>
                        ...
                        <div class="row g-4">
                                    ...
                                    <div class="input-container">
                                        <select name="ship_id" class="modern-form-control modern-form-select @error('ship_id') is-invalid @enderror" required>
                                            ...
                                            @foreach($ships as $ship)
                                                <option value="{{ $ship->id }}" {{ old('ship_id', $hotjob->ship_id) == $ship->id ? 'selected' : '' }}>{{ $ship->ship_name }}</option>
                                            @endforeach
                                        </select>
                                        <div class="validation-icon">
                                            <i class="bx bx-check-circle valid-icon"></i>
                                ...
                                <div class="modern-form-group">
                                    ...
                                    <div class="input-container">
                                        <input type="text"
                                               ...
                                               value="{{ old('joiningdate', $hotjob->joiningdate ? \Carbon\Carbon::parse($hotjob->joiningdate)->format('d-m-Y') : '') }}"
                                    ...
                                    <div class="input-container">
                                        <select name="nationality" class="modern-form-control modern-form-select @error('nationality') is-invalid @enderror" required>
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{ $country->country_name }}" {{ old('nationality', $hotjob->nationality) == $country->country_name ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="modern-form-group">
                                    ...
                                    <div class="row g-2">
                                        @php
                                            $expParts = explode(' ', $hotjob->experience);
                                            $expCount = old('experience_count', $expParts[0] ?? '');
                                            $expType = old('experience_type', $expParts[1] ?? 'Years');
                                        @endphp
                                        <div class="col-6">
                                            <select name="experience_count" class="modern-form-control @error('experience_count') is-invalid @enderror" required>
                                                <option value="">Select</option>
                                                @for($i=0; $i<=30; $i++)
                                                    <option value="{{ $i }}" {{ $expCount == $i ? 'selected' : '' }}>{{ $i }}</option>
                                                @endfor
                                            </select>
                                        ...
                                        <div class="col-6">
                                            <select name="experience_type" class="modern-form-control @error('experience_type') is-invalid @enderror" required>
                                                <option value="Months" {{ $expType == 'Months' ? 'selected' : '' }}>Months</option>
                                                <option value="Years" {{ $expType == 'Years' ? 'selected' : '' }}>Years</option>
                                            </select>
                                ...
                                <div class="modern-form-group">
                                    ...
                                    <div class="input-container">
                                        <textarea name="description"
                                                  ...
                                                  required>{{ old('description', $hotjob->description) }}</textarea>
                                        <div class="validation-icon">
                                            <i class="bx bx-check-circle valid-icon"></i>
                                            <i class="bx bx-error-circle invalid-icon"></i>
                                        </div>
                                ...
                                <div class="modern-form-group">
                                    ...
                                    <div class="input-container">
                                        <input type="text"
                                               ...
                                               value="{{ old('expiry_date', $hotjob->expiry_date ? \Carbon\Carbon::parse($hotjob->expiry_date)->format('d-m-Y') : '') }}"
                                               placeholder="DD-MM-YYYY"
                                               required
                                               autocomplete="off">
                                        <div class="validation-icon">
                                ...
                                <div class="modern-form-group">
                                    ...
                                    <div class="input-container">
                                        <select name="status" class="modern-form-control modern-form-select @error('status') is-invalid @enderror" required>
                                            <option value="pending" {{ old('status', $hotjob->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                            <option value="active" {{ old('status', $hotjob->status) == 'active' ? 'selected' : '' }}>Active</option>
                                            <option value="expired" {{ old('status', $hotjob->status) == 'expired' ? 'selected' : '' }}>Expired</option>
                                        </select>
                                        <div class="validation-icon">
                                            <i class="bx bx-check-circle valid-icon"></i>
                                            <i class="bx bx-error-circle invalid-icon"></i>
                                    ...
                                    <div class="compact-radio-group">
                                        <div class="compact-radio-option">
                                            <input type="radio"
                                                ...
                                                {{ old('withsms', $hotjob->withsms) == 'yes' ? 'checked' : '' }}>
                                        ...
                                        <div class="compact-radio-option">
                                            <input type="radio"
                                                ...
                                                value="no"
                                                class="compact-radio-input"
                                                {{ old('withsms', $hotjob->withsms) == 'no' ? 'checked' : '' }}>
                                ...
                                <div class="modern-form-group">
                                    ...
                                    <div class="input-container">
                                        <input type="text"
                                               ...
                                               value="{{ old('posted_by_name', $hotjob->posted_by_name) }}"
                                ...
                                <div class="modern-form-group">
                                    ...
                                    <div class="input-container">
                                        <input type="email"
                                               ...
                                               value="{{ old('posted_by_email', $hotjob->posted_by_email) }}"
                                    ...
                                    <div class="mobile-input-group">
                                        <div class="country-code-container">
                                            <select name="posted_by_country_code"
                                                    ...
                                                    required>
                                                <option value="+91" {{ old('posted_by_country_code', $hotjob->posted_by_country_code ?? '+91') == '+91' ? 'selected' : '' }}>🇮🇳 +91</option>
                                                <option value="+1" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+1' ? 'selected' : '' }}>🇺🇸 +1</option>
                                                <option value="+44" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+44' ? 'selected' : '' }}>🇬🇧 +44</option>
                                                <option value="+971" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+971' ? 'selected' : '' }}>🇦🇪 +971</option>
                                                <option value="+965" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+965' ? 'selected' : '' }}>🇰🇼 +965</option>
                                                <option value="+974" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+974' ? 'selected' : '' }}>🇶🇦 +974</option>
                                                <option value="+968" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+968' ? 'selected' : '' }}>🇴🇲 +968</option>
                                                <option value="+973" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+973' ? 'selected' : '' }}>🇧🇭 +973</option>
                                                <option value="+966" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+966' ? 'selected' : '' }}>🇸🇦 +966</option>
                                                <option value="+60" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+60' ? 'selected' : '' }}>🇲🇾 +60</option>
                                                <option value="+65" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+65' ? 'selected' : '' }}>🇸🇬 +65</option>
                                                <option value="+852" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+852' ? 'selected' : '' }}>🇭🇰 +852</option>
                                                <option value="+81" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+81' ? 'selected' : '' }}>🇯🇵 +81</option>
                                                <option value="+82" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+82' ? 'selected' : '' }}>🇰🇷 +82</option>
                                                <option value="+86" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+86' ? 'selected' : '' }}>🇨🇳 +86</option>
                                                <option value="+33" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+33' ? 'selected' : '' }}>🇫🇷 +33</option>
                                                <option value="+49" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+49' ? 'selected' : '' }}>🇩🇪 +49</option>
                                                <option value="+31" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+31' ? 'selected' : '' }}>🇳🇱 +31</option>
                                                <option value="+47" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+47' ? 'selected' : '' }}>🇳🇴 +47</option>
                                                <option value="+30" {{ old('posted_by_country_code', $hotjob->posted_by_country_code) == '+30' ? 'selected' : '' }}>🇬🇷 +30</option>
                                            </select>
                                        </div>
                                        <div class="mobile-input-container">
                                            <input type="tel"
                                                   ...
                                                   value="{{ old('posted_by_mobile', $hotjob->posted_by_mobile) }}"
...
$(function(){
    ...
    flatpickr("#joiningdate", {
        dateFormat: "d-m-Y",
        allowInput: true,
        defaultDate: "{{ old('joiningdate', $hotjob->joiningdate ? \Carbon\Carbon::parse($hotjob->joiningdate)->format('d-m-Y') : '') }}"
    });
    flatpickr("#expiry_date", {
        dateFormat: "d-m-Y",
        allowInput: true,
        defaultDate: "{{ old('expiry_date', $hotjob->expiry_date ? \Carbon\Carbon::parse($hotjob->expiry_date)->format('d-m-Y') : '') }}"
    });
    ...
    $('form').on('submit', function() {
        ...
        $submitBtn.find('span').text('Updating Job...');
    });
