                    ...
                    <div class="row g-4 align-items-end">
                        ...
                        <div class="col-md-3">
                            <div class="modern-form-group">
                                ...
                                <select id="search_field" name="search_field" class="modern-form-control">
                                    <option value="">Select Search Options</option>
                                    <option value="id" {{ request('search_field')=='id' ? 'selected' : '' }}>ID</option>
                                    <option value="indos" {{ request('search_field')=='indos' ? 'selected' : '' }}>Indos</option>
                                    ...
                                    <option value="date_of_availability" {{ request('search_field')=='date_of_availability' ? 'selected' : '' }}>Date Of Availability</option>
                                    <option value="present_rank" {{ request('search_field')=='present_rank' ? 'selected' : '' }}>Present Rank</option>
                                    <option value="post_applied_for" {{ request('search_field')=='post_applied_for' ? 'selected' : '' }}>Post Applied For</option>
                                    <option value="mobile" {{ request('search_field')=='mobile' ? 'selected' : '' }}>Mobile</option>
                                </select>
                            </div>
                        </div>

                        {{-- Dynamic Search Value Inputs --}}
                        <div class="col-md-4" id="search_value_container">
                            <input
                                type="text"
                                name="search_value"
                                id="search_value"
                            ...
                            <div id="search_rank_shiptype_input" style="display:none;">
                                <div class="row g-2">
                                    <div class="col-6">
                                        <select name="search_rank" class="modern-form-control">
                                            <option value="">Select Rank</option>
                                            @foreach($ranks as $r)
                                                <option value="{{ $r->id }}" {{ request('search_rank')==$r->id ? 'selected' : '' }}>{{ $r->rank }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-6">
                                        <select name="search_shiptype" class="modern-form-control">
                                            <option value="">Select Ship Type</option>
                                            @foreach($shiptypes as $s)
                                                <option value="{{ $s->id }}" {{ request('search_shiptype')==$s->id ? 'selected' : '' }}>{{ $s->ship_name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-2 d-flex gap-2">
                            ...
                            <a href="{{ route('admin.candidates.index') }}" class="enterprise-btn btn-secondary" style="background:#f3f4f6; color:#374151; border-color:#d1d5db;">
                ...
                <div class="table-container">
                    <table class="enterprise-data-table">
                        <thead class="table-header-section">
                            <tr>
                                <th class="table-header-cell no-column">#</th>
                                <th class="table-header-cell id-column">ID & Documents</th>
                                <th class="table-header-cell dates-column">Registration Date</th>
                                <th class="table-header-cell name-column">Name & Details</th>
                                <th class="table-header-cell age-column">Age</th>
                                <th class="table-header-cell mobile-column">Mobile</th>
                                <th class="table-header-cell rank-column">Present Rank</th>
                                <th class="table-header-cell applied-column">Applied For</th>
                                <th class="table-header-cell ship-column">Last Ship</th>
                                <th class="table-header-cell actions-column">Actions</th>
                            </tr>
                        </thead>
                                        ...
                                        <div class="dates-display">
                                            ...
                                            @if(optional($candidate->resume)->date_of_availability)
                                                <div class="availability-date">
                                                    <small class="availability-label">Availability:</small>
...
$(document).ready(function() {
    ...
    {{-- Dynamic search input handling --}}
    function updateSearchInput() {
    var field = $('#search_field').val();
    $('#search_value').show();
    $('#search_date').hide();
    $('#search_rank_shiptype_input').hide();

    if (['id','indos','name','email','mobile',''].includes(field)) {
        $('#search_value').attr('placeholder', 'Enter value...');
        $('#search_value').show();
    } else if (field === 'date_of_availability') {
        $('#search_value').hide();
        $('#search_date').show();
    } else if (field === 'present_rank' || field === 'post_applied_for') {
        $('#search_value').hide();
        $('#search_rank_shiptype_input').show();
    }
}
$(document).ready(function(){
    updateSearchInput();
    $('#search_field').on('change', updateSearchInput);
});
    // Datepicker for DD-MM-YYYY
    $('#date_picker_input').on('focus', function(){
        $(this).attr('type', 'date');
    }).on('blur', function(){
