    ...
    protected function createCandidate(array $data): User
    {
        return DB::transaction(function () use ($data) {
            ...
            $profile = CandidateProfile::create([
                ...
                // optionally save skipped steps as JSON
                'skipped_steps' => isset($data['skipped']) ? json_encode($data['skipped']) : null,
            ]);
            ...
            if (!empty($data['profile_pic_file']) && $data['profile_pic_file'] instanceof \Illuminate\Http\UploadedFile) {
                try {
                    ...
                    $filename = time() . '_' . Str::slug(pathinfo($data['profile_pic_file']->getClientOriginalName(), PATHINFO_FILENAME))
            ...
            $resume = CandidateResume::create([
                ...
                'usa_visa' => isset($data['usa_visa']) ? ($data['usa_visa'] === 'yes' ? 1 : 0) : null,
                'cdc_nationality' => $data['cdc_nationality'] ?? null,
                'cdc_no' => $data['cdc_number'] ?? null, // FIX: use cdc_number
            ...
            if (!empty($data['sea_service']) && is_array($data['sea_service'])) {
                foreach ($data['sea_service'] as $entry) {
                    SeaServiceDetail::create([
                        ...
                        'grt_value'    => $entry['grt_value'] ?? null,
                        'bhp'          => $entry['bhp'] ?? null,
                    ]);
                }
            }

            // Profile completion (pass skipped steps)
            $skipped = $data['skipped'] ?? [];
            $this->updateProfileCompletion($profile, $resume, $skipped);

            return $user;
        });
    }

    protected function updateProfileCompletion(CandidateProfile $profile, CandidateResume $resume, array $skipped = []): void
    {
        $steps = [
            'personal' => 0,
            'profile' => 0,
        ...
        // Remove skipped steps from calculation
        foreach ($skipped as $step => $isSkipped) {
            if ($isSkipped) unset($steps[$step]);
        }

        $completed = array_sum($steps);
        $percent = count($steps) ? (int) round(($completed / count($steps)) * 100) : 0;
